...
class downModis:
    ...
    def __init__(self, destinationFolder, password=None, user=None,
        ...
        if not GDAL and checkgdal:
            ...
            GDAL = False
        self.dirData = []
        # the possibility to store streamdata in dictionary
        #self.dataIO=dict()
...
#                sys.exit()
    ...
    def downloadFile(self, filDown, filHdf, day, bytestream=False):
        """Download a single file

           :param str filDown: name of the file to download
           :param str filHdf: name of the file to write to or key of dict
           :param str day: the day in format YYYY.MM.DD
        """
        if bytestream:
            self._downloadbyteIOHTTP(filDown,day)
        else:
            self._downloadFileHTTP(filDown, filHdf, day)
            
    def _downloadbyteIOHTTP(self,filDown,day):
        """
        The tool to get MODIS data as Bytestream
        :param str filHdf: The tile of the dictionary
        """
        url = urljoin(self.url, self.path, day, filDown)
        try:  # download and write the file
            req = urllib.request.Request(url, headers=self.http_header)
            http = urllib.request.urlopen(req)
            self.dataIO=BytesIO(http.read())
